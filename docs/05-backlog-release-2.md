...
| Pedido | Qué implica | Tamaño |
|---|---|---|
| ✅ Casilla de observaciones por operación | Construido el 2026-09-21, al hacer testing de regresión de toda la app: se detectó que había quedado sin hacer (el campo ya existía de punta a punta desde Fase 1 — `registro_operacion.observacion` en el schema y la Server Action ya lo leían — sólo faltaba el `<textarea>` en `/taller/[otPiezaId]`, verificado end-to-end) | **XS — hecho** |
